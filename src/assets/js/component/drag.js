export default class Drag {
  ...
  init() {
    ...
    this.areas.forEach(item => {
      ...
      this.areaPsData.push({
        x: [area_rect.left + window.scrollX, area_rect.left + area_rect.width + window.scrollX],
        y: [area_rect.top + window.scrollY, area_rect.top + area_rect.height + window.scrollY],
        target: item,
        info: area_rect,
      });
    });
  ...
  actEventStart(e) {
    ...
    const y = (isTouchEvent ? e.targetTouches[0].clientY : e.clientY) + window.scrollY;
    const x = (isTouchEvent ? e.targetTouches[0].clientX : e.clientX) + window.scrollX;

    const item_rect = e.currentTarget.getBoundingClientRect();
    const centerX = item_rect.left + item_rect.width / 2 + window.scrollX;
    const centerY = item_rect.top + item_rect.height / 2 + window.scrollY;
    ...
    const actMove = e => {
      e.preventDefault();
      const y_m = (isTouchEvent ? e.targetTouches[0].clientY : e.clientY) + window.scrollY;
      const x_m = (isTouchEvent ? e.targetTouches[0].clientX : e.clientX) + window.scrollX;
      ...
      if (isClone) {
        ...
        const m_x = x_m - (target_rect.left + window.scrollX) - (rect_item.width / 2);
        const m_y = y_m - (target_rect.top + window.scrollY) - (rect_item.height / 2);
    ...
    const actMoveEnd = e => {
      ...
      const end_ps = {
        x: [end_rect.left + window.scrollX, end_rect.left + end_rect.width + window.scrollX],
        y: [end_rect.top + window.scrollY, end_rect.top + end_rect.height + window.scrollY]
      };
