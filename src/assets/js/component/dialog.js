...
export default class Dialog {
  ...
  init() {
    ...
    switch (this.type) {
      case "modal":
        this.area = document.querySelector('.area-dialog[data-area="body"]');
        if (!this.area) {
          document
            .querySelector("body")
            .insertAdjacentHTML(
              "beforeend",
              `<div class="area-dialog" data-area="body"></div>`
            );
          this.area = document.querySelector('.area-dialog[data-area="body"]');
        }
        this.initDialog();
      ...
      case "system":
        ...
        break;
      case "toast":
        this.area = document.querySelector('.area-toast[data-area="body"]');
        if (!this.area) {
          document
            .querySelector("body")
            .insertAdjacentHTML(
              "beforeend",
              `<div class="area-toast" data-area="body"></div>`
            );
          this.area = document.querySelector('.area-toast[data-area="body"]');
        }
   ...
   */
  addToastMouseEvents() {
    // 바인딩된 함수들을 저장 (나중에 제거하기 위해)
    this.boundToastMouseOver = this.handleToastMouseOver.bind(this);
    this.boundToastMouseLeave = this.handleToastMouseLeave.bind(this);

    // 마우스 이벤트 리스너 등록
    this.elToast.addEventListener("mouseover", this.boundToastMouseOver);
  ...
  removeToastMouseEvents() {
    if (this.elToast && this.boundToastMouseOver && this.boundToastMouseLeave) {
      ...
      this.elToast.removeEventListener("mouseleave", this.boundToastMouseLeave);

      // 바인딩된 함수 참조 정리
      this.boundToastMouseOver = null;
      this.boundToastMouseLeave = null;

      logger.debug("Toast mouse events removed", null, "Dialog");
    }
  }
  toastAnimationend() {
    // transitionend 이벤트 리스너 제거
    this.elToast.removeEventListener(
      "transitionend",
      this.boundToastAnimationend
    );
    ...
    logger.debug("Toast removed from DOM", null, "Dialog");
  }
  toastHide() {
    // 타이머 정리
    if (Timeouts[this.randomID]) {
  ...
  initSystem() {
    ...
    <div class="ui-dialog" 
      ...
      <div class="ui-dialog--wrap" role="document" tabindex="-1" data-dialog-item="wrap">
        ${
          this.title
            ? '<div class="ui-dialog--header"><h2 id="' +
              this.id +
              '-label">' +
              this.title +
              "</h2></div>"
        ...
        <div class="ui-dialog--footer">
          ${
            this.cancelText
              ? '<button type="button" class="btn-base" data-dialog-button="cancel">' +
                this.cancelText +
                "</button>"
              : ""
          }
          ${
            this.confirmText
              ? '<button type="button" class="btn-base" data-dialog-button="confirm">' +
                this.confirmText +
                "</button>"
              : ""
          }
  ...
  initDialog() {
    if (this.src && !this.dialog) {
      loadContent({
        ...
        .catch((err) =>
          logger.error("Error loading modal content", err, "Dialog")
        );
    } else {
      this.buildDialog();
    }
  ...
  buildDialog() {
    ...
    if (this.dim) {
      ...
      this.dimClick &&
        this.el_dim.addEventListener("click", this.handleDimClick.bind(this));
    }

    //extend
    this.extend &&
      this.dialogWrap.insertAdjacentHTML(
        "afterbegin",
        '<div data-dialog-item="extend"></div>'
      );
  ...
  zIndexUp() {
    ...
    for (let i = thisZindex; i < zIndex; i++) {
      const item = document.querySelector(
        `[data-dialog][aria-hidden="false"][data-zindex="${i + 1}"]`
      );
      if (item) {
  ...
  moveStart(e) {
    ...
    const initalX = el_this.dataset.translateX
      ? Number(el_this.dataset.translateX)
      : 0;
    const initalY = el_this.dataset.translateY
      ? Number(el_this.dataset.translateY)
      : 0;
    ...
    const dragMove = (e) => {
      ...
      el_this.style.transform = `translate(${initalX + deltaX}px, ${
        initalY + deltaY
      }px)`;
  ...
  extendStart(e) {
    ...
    const dragMove = (e) => {
      ...
      if (isDragState) {
        if (
          Math.abs(deltaY) > 10 &&
          Math.abs(x - x_m) < Math.abs(deltaY) &&
          deltaY < 0
        ) {
    ...
    const dragEnd = () => {
      ...
      const reDrag = (e) => {
        ...
        const reDragEnd = () => {
          ...
          if (
            _t < 1 &&
            _y - _y_m < 0 &&
            Math.abs(_x - _x_m) < Math.abs(_y - _y_m)
          ) {
            this.dialogWrap.removeEventListener("touchstart", reDrag);
            this.dialogWrap.addEventListener(
              "touchstart",
              this.boundExtendStart,
              { passive: true }
            );
          } else {
      ...
      const dragScope = 60;
      const fullMaxHeight =
        "max-height:100dvh !important; overflow-y: hidden !important; height: 100dvh !important;";
      if (
        Math.abs(y_m - y) > dragScope &&
        isMove &&
        ...
        const dragCloseBtn = this.dialog.querySelector(
          '[data-dialog-drag="close"]'
        );
        isDragState = true;
        ...
        this.dialogWrap.addEventListener("transitionend", () => {
          ...
          if (hasScroll) {
            this.dialogWrap.removeEventListener(
              "touchstart",
              this.boundExtendStart,
              { passive: true }
            );
        ...
        (Math.abs(y_m - y) < dragScope || y_m === undefined) &&
        this.dialog.dataset.state === "drag-full"
      ) {
        ...
        const el_extend = this.dialog.querySelector(
          '[data-dialog-item="extend"]'
        );
        el_extend.removeEventListener("touchstart", this.boundExtendStart, {
          passive: true,
        });
    ...
    if (this.type === "toast") {
      ...
      if (this.dialog === null) {
        ...
        const currentModal = document.querySelector(
          '[data-dialog][aria-hidden="false"][data-current="true"]'
        );
        ...
        if (this.extend) {
          ...
          el_extend.addEventListener("touchstart", this.boundExtendStart, {
            passive: true,
          });
          el_extend.addEventListener("mousedown", this.boundExtendStart, {
            passive: true,
          });
        }
        if (this.move) {
  ...
  hide(opt) {
    ...
    //열린 모달 재설정
    const openModals = document.querySelectorAll(
      '[data-dialog][aria-hidden="false"]:not([data-type="toast"])'
    );
    const zIndex = openModals.length;
    ...
    //다음선택 모달 설정
    const currentModal = document.querySelector(
      `[data-dialog][aria-hidden="false"][data-zindex="${zIndex}"]`
    );
    if (currentModal) {
