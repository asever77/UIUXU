...
export default class Tab {
  ...
  init() {
    ...
    if (this.#renderMode === "static") {
      ...
      if (!this.#loadAll) {
        ...
        if (this.#scroll) {
          ...
          this.#data.forEach((item, index) => {
            const n = index + 1;
            const targetPanel = this.#el_wrap.querySelector(
              `#${this.#id}-panel-${n}`
            );
            if (targetPanel) {
              ...
              loadContent({
                ...
                .catch((err) =>
                  console.error(
                    `Error loading tab content for panel ${n}:`,
                    err
                  )
                ); // 상세 에러 로깅
            }
    ...
    if (this.#scroll) {
      ...
      const rootMarginTop =
        (window.innerHeight - this.#scrollOffsetTop) * -1 + "px";
      this.#scrolltrigger = new ScrollTrigger({
        ...
        callback: (element) => {
          const tab = document.querySelector(
            `[aria-controls="${element.target.id}"]`
          );
          const name = tab.dataset.tabName;
          const selected = document.querySelector(
            `[data-tab-name="${name}"][aria-selected="true"]`
          );
  ...
  #loadPanel(n, tabID) {
    ...
    loadContent({
      ...
      .catch((err) =>
        console.error(`Error loading tab content for panel ${n}:`, err)
      ); // 상세 에러 로깅
  }
  ...
  #handleToggle(e) {
    ...
    const tabSelected = _wrap.querySelector(
  ...
  expanded(id) {
    ...
    const tabSelected = _wrap.querySelector(
      `[data-tab-name="${tabName}"][role="tab"][aria-selected="true"]`
    ); // 현재 선택된 탭 버튼
    const panelWrap = document.querySelector(`[data-tab-wrap="${tabName}"]`); // 탭 패널 컨테이너
    const panelSelected = panelWrap.querySelector(
      `[data-tab-name="${tabName}"][role="tabpanel"][aria-expanded="true"]`
    ); // 현재 확장된 탭 패널
    const currentPanel = panelWrap.querySelector(
      `[data-tab-name="${tabName}"][role="tabpanel"][aria-labelledby="${tabID}"]`
    ); // 활성화할 탭에 연결된 패널
  ...
  destroy() {
    ...
    if (
      this.#keyNavigator &&
      typeof this.#keyNavigator.destroy === "function"
    ) {
    ...
    if (
      this.#scrolltrigger &&
      typeof this.#scrolltrigger.destroy === "function"
    ) {
    ...
    if (
      this.#smoothScroll &&
      typeof this.#smoothScroll.destroy === "function"
    ) {
